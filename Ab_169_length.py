...
--Authors : Yatindrapravanan Narasimhan--
--27-Mar-21--
"""
...
path = 'C:/Users/user/OneDrive/Desktop/python/Thamo Sir/Pakistan/FASTA (1)'  # Change the path as per your folder
temp = path.split("/")
...
in_dir = path + '//'  # for using a variable instead of the direct name of the input folder (in line 23)
...
for n in filenames:
    ...
    for record in SeqIO.parse(filename_read, 'fasta'):
        if len(record.seq) == 169:  # Storing info of sequences with seq.len. 169 in out_169
            ...
            Id_169 = "|".join(Id_169.split("|")[0:2])
            Id_Seq.append({Id_169: Seq_169})
    ...
    if __name__ == "__main__":
        print("\n" + filename_write + ":")
        print("There are", int(len(out_169) / 2), 'sequences having seq. length = 169, and these are written in "' + filename_write + '_len_169.csv".')
        print("There are", int(len(out_not_169) / 3), 'sequences NOT having seq. length = 169, and these are written in"' + filename_write + '_len_not_169.csv".')
